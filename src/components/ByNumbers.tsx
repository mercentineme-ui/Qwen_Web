...
/* rapid mechanical count — assembles in ~650ms with easing, never a long zero-roll */
        ...
        <span className="flex items-end gap-[3px]" aria-hidden>
          {[0, 1, 2, 3].map((k) => (
            <span key={k} className={`w-[3px] ${start ? "tick-grow" : "scale-x-0"}`}
              style={{ height: `${5 + k * 3}px`, background: k === 3 ? "var(--crimson)" : "var(--ink2)", opacity: 0.7, animationDelay: `${delay + k * 70}ms` }} />
          ))}
        </span>
      </div>
