...
/* ================= 2×2 CUBE (approved — geometry/colors/motion/interaction preserved) ================= */
                ...
                {FACES.map((f) => {
                  ...
                  return (
                    <div key={f.k} style={{
                      ...
                      background: crim ? "var(--crim-panel)" : body,
...
export default function AILab() {
  ...
  /* KNOW MORE from Show Reel lands here and arms the featured video */
  useEffect(() => {
    const onArm = () => { setArmed(true); window.setTimeout(() => setArmed(false), 6000); };
    ...
    <section id="ailab" className="relative py-20 lg:py-28 scroll-mt-20">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8">
        <div className="mat-outer mat-texture rounded-xl p-5 sm:p-8 lg:p-12 overflow-hidden">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-end justify-between gap-6 border-b pb-5" style={{ borderColor: "var(--m-line)" }}>
                <div className="flex items-end gap-4 min-w-0">
                  <span className="f-mono text-[11px] sm:text-xs tracking-[0.3em] text-[var(--crim-panel)] pb-2">/04</span>
                  <h2 className="f-display leading-[0.95] text-[clamp(2rem,5.2vw,4.2rem)] tracking-wide whitespace-nowrap">
                    ...
                    <span className="text-[var(--crim-panel)]">LAB</span>
                  </h2>
                </div>
          ...
          {/* GHOST.EXE — compact editorial project dossier (tools listed ONCE) */}
              ...
              <div className="grid lg:grid-cols-[1.35fr_0.65fr] gap-7 lg:gap-10">
                <div className="min-w-0">
                  ...
                  <dl className="mt-6 grid sm:grid-cols-3 border-t" style={{ borderColor: "var(--m-line)" }}>
                    {[
                      ...
                      <div key={k} className={`py-3.5 sm:px-4 ${i > 0 ? "sm:border-l border-t sm:border-t-0" : ""}`} style={{ borderColor: "var(--m-line)" }}>
                ...
                <div className="lg:border-l lg:pl-8" style={{ borderColor: "var(--m-line)" }}>
                  ...
                  <ul className="mt-3.5 flex flex-col">
                    {lab.tools.map((t, i) => (
                      <li key={t} className="group flex items-center gap-3 py-2 border-b last:border-b-0"
          ...
          <Reveal className="mt-8">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="f-mono text-[10px] tracking-[0.2em] text-[var(--crim-panel)]">FULL CUT</span>
            </div>
            <div className={`relative rounded-lg overflow-hidden transition-shadow duration-500 ${armed ? "shadow-[0_0_0_2px_var(--crim-panel),0_18px_40px_-24px_rgba(34,35,40,0.9)]" : ""}`}>
              <MediaSlot item={lab.video} ratio="16/9" className="mat-inner" showLabel={false} onClick={() => setVideoOpen(true)} />
              {armed && (
                <span className="absolute top-3 left-3 z-10 f-tech font-bold text-[9px] tracking-[0.26em] px-2.5 py-1.5 rounded-lg bg-[var(--crim-panel)] text-[#DDDDD8] live-blink">
          ...
          {/* 8 generic upload slots — click opens fullscreen carousel */}
          <Reveal className="mt-8">
            <div className="flex items-center justify-between mb-3">
      ...
      {tileView !== null && (
        <FullscreenViewer
          ...
          index={tileView}
          ratio="16/9"
          onClose={() => setTileView(null)}
          setIndex={setTileView}
        />
      )}
    </section>
  );
