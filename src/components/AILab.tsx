...
/* ================= 2×2 CUBE ENGINE (approved — geometry / colors / motion preserved) ================= */
...
function Cube() {
  ...
  const play = (m: Move, dur: number, then?: () => void) => {
    ...
    window.setTimeout(() => { commit(m); setAnim(null); busy.current = false; then?.(); }, d + 70);
  };
    ...
    <section id="ailab" className="relative py-20 lg:py-28 scroll-mt-20">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8">
        <div className="mat-outer mat-texture rounded-xl p-5 sm:p-8 lg:p-12 overflow-hidden">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-end justify-between gap-6 border-b pb-5" style={{ borderColor: "var(--m-line)" }}>
                <div className="flex items-end gap-4 min-w-0">
                  <span className="f-mono text-[11px] sm:text-xs tracking-[0.3em] text-[var(--crimson)] pb-2">/04</span>
          ...
          {/* 8 generic upload slots — no categories, no specifications */}
          <Reveal className="mt-8">
            <div className="flex items-center justify-between mb-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
              {lab.images.map((im) => (
                <MediaSlot key={im.id} item={im} ratio="16/9" className="mat-inner" showLabel={false} />
              ))}
            </div>
          </Reveal>
        </div>
