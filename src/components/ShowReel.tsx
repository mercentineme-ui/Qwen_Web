...
/* one short synthesized "dropped metal tea tin" ping (~1s, played once) */
function playTin() {
  try {
  ...
  return (
    <div className="px-4 sm:px-6 pb-6 pt-2 flex items-center gap-4 sm:gap-5">
      ...
      <div ref={railRef}
        ...
        onPointerDown={(e) => { dragging.current = true; setLive(true); (e.currentTarget as HTMLElement).setPointerCapture?.(e.pointerId); scrub(e.clientX); }}
        ...
        <span className={`absolute top-1/2 -translate-y-1/2 w-14 h-[26px] rounded-lg border-[1.5px] mat-texture flex items-center justify-center gap-[3px] ${live ? "" : "transition-[left] duration-300"}`}
          style={{
...
export default function ShowReel() {
  ...
  /* sea creature event — idle mid-voyage → slow approach + capture */
  useEffect(() => {
  ...
  /* island event — dock beside the island, flag rises, LAND AHOY!, one tin ping */
  useEffect(() => {
    if (arrived && !tinFired.current) { tinFired.current = true; playTin(); }
    ...
    <section id="showreel" className="relative py-20 lg:py-28 scroll-mt-20 overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8">
        <SectionHead
          ...
          meta="05 PORTRAITS · 06 LANDSCAPES"
        />

        {/* ---------- PORTRAIT STUDIES — 9:16 (exactly 5) ---------- */}
        ...
        {/* ---------- WIDE FRAMES — 16:9 (exactly 6) + ODYSSEY SEA SYSTEM ---------- */}
        <Reveal className="mt-12">
          <div className="mat-page-card mat-texture rounded-xl border border-[var(--line)] overflow-hidden">
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 pt-4">
              <span className="f-mono text-[10px] tracking-[0.28em] text-[var(--ink2)]">WIDE FRAMES — 16:9 · DRAG HORIZONTALLY</span>
            ...
            {/* sea / ship visual — the ship stays secondary to the media */}
            <div className="relative h-[138px] sm:h-[152px] mx-2 sm:mx-4 mt-2 overflow-hidden text-[var(--ink)]">
              ...
              {/* sea creature — dragon head + squid tentacles, emerges from the LEFT when idle */}
              ...
              {/* ODYSSEY — clean 2D line art, docks BESIDE the island at full voyage */}
            ...
            <div ref={viewRef}
              className="track-drag overflow-hidden px-4 sm:px-6 pb-4 pt-1 select-none"
              ...
              <div ref={innerRef} className="flex gap-4 sm:gap-5 w-max"
                ...
                {landscapes.map((l, i) => (
                  <div key={l.id} className="relative w-[240px] sm:w-[340px] lg:w-[420px] shrink-0">
                    ...
                    {i === 0 && (
                      /* first landscape — VIEW + KNOW MORE */
                      <div className="absolute inset-0 z-10 group/first">
                        <div className="absolute inset-0 opacity-0 group-hover/first:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3"
                          ...
                          <button onClick={() => setView({ group: "l", i: 0 })} className="btn btn-crimson !px-4 !py-2.5 text-[10px]">
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M6 4l14 8-14 8z" /></svg>
                            VIEW
                          </button>
                          <button onClick={knowMore} className="btn btn-ghost border-[var(--ink)] text-[var(--ink)] !px-4 !py-2.5 text-[10px] bg-[var(--page)]">
                            KNOW MORE ↓
                          </button>
                        </div>
            ...
            {/* ONE physical track control — below the landscape media */}
