...
export function useInView<T extends HTMLElement>(threshold = 0.3): [React.RefObject<T | null>, boolean] {
  ...
  useEffect(() => {
    const el = ref.current;
    if (!el) return;
    const io = new IntersectionObserver((es) => es.forEach((e) => e.isIntersecting && setInView(true)), { threshold });
    io.observe(el);
    return () => io.disconnect();
  }, [threshold]);
  return [ref, inView];
...
/* ============================================================
   ...
   small technical label chip + large editorial heading + description
   ============================================================ */
      ...
      <div id={id} className="scroll-mt-28">
        <div className="flex items-center justify-between gap-6">
          <span className="f-mono text-[10px] sm:text-[11px] tracking-[0.26em] inline-flex items-center gap-2.5 px-3 py-2 border border-[var(--line)] rounded-[6px]">
            ...
            {namePart && <><span className="text-[var(--crimson)]">—</span><span className="text-[var(--ink)]">{namePart}</span></>}
          </span>
          {meta && (
            <span className="f-mono text-[10px] sm:text-[11px] tracking-[0.22em] text-[var(--ink2)] hidden md:block shrink-0">{meta}</span>
          )}
        </div>
        <h2 className={`f-display leading-[0.95] tracking-wide mt-4 ${
          huge ? "text-[clamp(3rem,8vw,6.2rem)]"
            : long ? "text-[clamp(1.5rem,3.7vw,3.15rem)]"
    ...
    <div className="absolute inset-0 mat-page-card"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 p-3 text-center">
        ...
        {(item.emptyLines ?? [isVideo ? "ADD VIDEO" : "ADD IMAGE"]).map((l, i) => (
          <span key={i} className={`f-mono tracking-[0.3em] text-[9px] ${i === 0 ? "text-[var(--crimson)]" : "text-[var(--ink2)] opacity-75"}`}>{l}</span>
        ))}
      </div>
...
/* ============================================================
   MEDIA SLOT — editable placeholder or loaded media.
   With onClick: hover lifts slightly + VIEW affordance → fullscreen.
   ============================================================ */
export function MediaSlot({
  ...
  return (
    <figure
      ...
      style={{
        aspectRatio: ratio,
        transition: onClick ? "transform .35s cubic-bezier(.3,.8,.3,1), border-color .35s ease, box-shadow .35s ease" : undefined,
      }}
      onMouseEnter={onClick ? (e) => {
        e.currentTarget.style.transform = "translateY(-3px)";
        e.currentTarget.style.borderColor = "color-mix(in srgb, var(--crimson) 55%, var(--line))";
        e.currentTarget.style.boxShadow = "0 16px 32px -20px rgba(34,35,40,0.6)";
      } : undefined}
    ...
    >
      ...
      {onClick && (
        <span className="absolute top-2.5 right-2.5 z-10 f-tech font-bold text-[9px] tracking-[0.22em] px-2.5 py-1.5 rounded-[6px] bg-[#DDDDD8] text-[#222328] opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center gap-1.5 shadow-[0_6px_16px_-8px_rgba(34,35,40,0.8)]">
          {isVideo ? (
            <svg width="9" height="9" viewBox="0 0 24 24" fill="currentColor"><path d="M6 4l14 8-14 8z" /></svg>
          ) : (
            <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6"><path d="M4 9V4h5M20 15v5h-5M4 4l6 6M20 20l-6-6" /></svg>
          )}
          VIEW
        </span>
      )}
...
/* ============================================================
   FULLSCREEN VIEWER — one reusable immersive overlay.
   White button boxes with black icons, visible in both themes.
   ‹ › NAVIGATE the carousel without closing · only ✕ closes.
   Closing restores exact page state — no reload, no reset.
   ============================================================ */
