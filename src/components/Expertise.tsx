  ...
  useEffect(() => {
    ...
    const iv = window.setInterval(() => {
      if (hover) {
        ...
        return; // frozen — resume keeps the exact position
      }
  ...
  return (
    <section id="expertise" className="relative py-20 lg:py-28 scroll-mt-20">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-8">
        {/* universal head — the editorial statement IS the main heading */}
        <SectionHead
        ...
        {/* twin module headings — same horizontal baseline */}
        <Reveal className="mt-10 grid lg:grid-cols-[1.05fr_0.95fr] gap-x-6 gap-y-8 lg:gap-x-10">
          <ModuleHeading tag="A" title="CAREER INFO" right={locked ? "LOCKED — CLICK NODE TO RELEASE" : hover ? "CYCLE PAUSED" : reduced ? "STATIC" : "AUTO CYCLE · 30S"} />
          <ModuleHeading tag="B" title="CAREER NODE MAP" right={<span className="text-[var(--ink)]">ACTIVE — {co.name}</span>} />
        </Reveal>

        {/* one integrated career composition */}
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-6 lg:gap-10 mt-6 items-stretch"
          onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>

          {/* ================= CAREER INFO ================= */}
          <Reveal className="min-h-0">
            <div className="mat-outer mat-texture rounded-xl p-5 sm:p-7 xl:p-8 lg:h-[660px] flex flex-col overflow-hidden">
              {/* company selector */}
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* cycle progress — freezes exactly where paused */}
                {!reduced && (
              ...
              <div className="mt-6 grid xl:grid-cols-[0.95fr_1.25fr] gap-6 lg:gap-8 flex-1 min-h-0">
                {/* dossier inner card — opposite material */}
                <div className="mat-inner mat-texture chamfer relative p-5 sm:p-6 flex flex-col min-h-0 overflow-hidden">
                  <div key={co.id} className="dossier-swap flex flex-col gap-3.5 min-h-0">
                    ...
                    <span className="f-mono text-[10px] tracking-[0.2em] text-[var(--m-sub)]">{co.date}</span>
                ...
                <div className="flex flex-col min-h-0">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="f-mono text-[9px] tracking-[0.2em] text-[var(--m-sub)]">{co.name}</span>
                  </div>
                  ...
                  <p className="mt-auto pt-4 f-mono text-[9px] tracking-[0.2em] text-[var(--m-sub)] hidden xl:block">
          ...
          <Reveal delay={0.1} className="min-h-0">
            <div className="mat-page-card mat-texture rounded-xl border border-[var(--line)] p-5 sm:p-7 xl:p-8 lg:h-[660px] flex flex-col overflow-hidden">
              <div className="flex items-center justify-between mb-5">
                <span className="f-mono text-[10px] tracking-[0.26em] text-[var(--ink2)]">SIGNAL ROUTE — CIRCUIT → TRUNK → NODE</span>
                <span className="f-mono text-[10px] tracking-[0.2em] text-[var(--crimson)] hidden sm:block">{co.num} ACTIVE</span>
              </div>

              <div className="grid grid-cols-[1fr_auto_1fr] items-stretch gap-x-0 sm:gap-x-6 flex-1 content-center">
                {/* LEFT — all four company node modules (left of the trunk) */}
                ...
                <div className="relative w-12 sm:w-16 mat-texture rounded-[6px] border-x-4 border-[var(--ink)] self-center" style={{ backgroundColor: "var(--sup2)", height: ROW_H * n }}>
                  {/* inset channel */}
              ...
              {/* map footer legend */}
              <div className="mt-5 pt-4 border-t border-[var(--line)] flex flex-wrap gap-x-8 gap-y-2 f-mono text-[9px] tracking-[0.22em] text-[var(--ink2)]">
